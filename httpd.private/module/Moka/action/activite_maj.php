...
$query = "UPDATE moka_activite SET code =:code, analytique_id =:analytique_id, client_id =:client_id, remuneration_type =:remuneration_type WHERE id = :id";
$stmt = $dbh->prepare($query);
$stmt->bindParam(':code', $data['code']);
$stmt->bindParam(':analytique_id', $data['analytique_id']);
$stmt->bindParam(':remuneration_type', $data['remuneration_type']);
$stmt->bindParam(':client_id',  $_SESSION['client_actif']);
$stmt->bindParam(':id',  $data['activite_id']);
$stmt->execute();
exit($responseHandler->sendResponse(true,'Activite mise à jour'));
?>