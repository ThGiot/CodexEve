...
$query = "SELECT * FROM  moka_activite WHERE client_id =:client_id AND id =:activite_id";
...
//Vérification que cet aactivite appartient au client
$query = "DELETE FROM moka_activite WHERE id =:activite_id";
$stmt = $dbh->prepare($query);
$stmt->bindParam(':activite_id', $data['activite_id']);
$stmt->execute();

$query = "DELETE FROM moka_activite_remuneration WHERE activite_id =:activite_id";
$stmt = $dbh->prepare($query);
$stmt->bindParam(':activite_id', $data['activite_id']);
$stmt->execute();
// Check if any rows are returned, which would indicate permission
exit($responseHandler->sendResponse(true, "Activitée supprimée."));
