...
try {
    $sql = "    SELECT *, YEAR(date) AS annee FROM moka_facture_correction_archive  mfca
                JOIN moka_facture mf ON mf.id = mfca.facture_id
                WHERE mfca.client_id =:client_id AND numero != 0 AND prestataire_id IS NOT NULL ORDER BY mfca.id DESC";
