...
try {
    const startDate = new Date(programme.getAttribute("start"));
    const endDate = new Date(programme.getAttribute("stop"));

    if (isNaN(startDate) || isNaN(endDate)) {
        console.warn("Invalid date found in XML:", programme);
        continue; // Ignore ce programme si les dates sont invalides
    }

    // Continue le traitement ici
} catch (e) {
    console.error("Error parsing dates:", e, programme);
    continue; // Ignore les programmes avec des erreurs de date
}
