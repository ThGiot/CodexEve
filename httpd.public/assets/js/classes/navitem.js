class NavItem {
    ...
    render() {
        return `<div class='col-4'>
                    <a class='d-block hover-bg-200 p-2 rounded-3 text-center text-decoration-none mb-3' onclick="moduleChange('${this.link}')">
...
class NavItemList {
    ...
    addItem(icon, title, link = "#!") {
        this.items.push(new NavItem(icon, title, link));
    }

    render() {
        let items_html = this.items.map(item => item.render()).join("");
        return `
